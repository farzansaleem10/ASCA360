...
async function fetchGoogleSheet(range) {
  ...
  if (!API_KEY || !SHEET_ID) {
    throw new Error(
      "Google Sheets API not configured. Please set GOOGLE_SHEETS_API_KEY and SHEET_ID in .env file."
    );
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch data from Google Sheets: ${errorData.error.message}`
      );
    }
