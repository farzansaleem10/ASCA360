  ...
  <div className="profile-content-card">
    <div className="profile-header">
      <img
        src={
          alumniData.photoUrl ||
          `https://placehold.co/100x100/1C1C1C/FFFFFF?text=${alumniData.firstName.charAt(
            0
          )}`
        }
        ...
        onError={(e) => {
          ...
          e.target.src = `https://placehold.co/100x100/1C1C1C/FFFFFF?text=${alumniData.firstName.charAt(
            0
          )}`;
        }}
      />
    ...
    <div className="profile-details">
      <div className="detail-group">
        <h4>Contact & Links</h4>
        <p>
          <strong>Graduation Year:</strong> {alumniData.graduationYear}
        </p>
        <p>
      ...
      {alumniData.currentWorkplace?.institution && (
        <div className="detail-group">
          ...
          <p>
            {alumniData.currentWorkplace.designation} at{" "}
            <strong>{alumniData.currentWorkplace.institution}</strong>
          </p>
        </div>
      )}

      {alumniData.experiences?.length > 0 &&
        alumniData.experiences.some((exp) => exp.institution) && (
          <div className="detail-group">
            <h4>Past Experience</h4>
            {alumniData.experiences.map((exp, index) =>
              exp.institution ? (
                <p key={index}>
                  {exp.designation} at <strong>{exp.institution}</strong> (
                  {exp.years})
                </p>
              ) : null
            )}
...
// --- Sub-Component for Managing Announcements ---
    ...
    try {
      ...
      if (response.ok) {
        const filtered = allAnnouncements.filter(
          (ann) =>
            ann.createdBy === `${alumniData.firstName} ${alumniData.lastName}`
        );
...
const CommunityBrowser = () => {
  ...
  const handleYearChange = (e) => {
    ...
    if (year) {
      setFilteredAlumni(
        allAlumni.filter((alumni) => alumni.graduationYear.toString() === year)
      );
    } else {
  ...
  return (
    <div className="community-browser-card">
            ...
            {filteredAlumni.map((alumni) => (
              <div key={alumni._id} className="alumni-card">
                <img
                  src={
                    alumni.photoUrl ||
                    `https://placehold.co/100x100/1C1C1C/FFFFFF?text=${alumni.firstName.charAt(
                      0
                    )}`
                  }
...
const AlumniDashboard = ({ userName, onLogout }) => {
    ...
    const fetchProfile = async () => {
      try {
        const response = await fetch(
          `${backendUrl}/alumni/profile/${userName}`
        );
        const data = await response.json();
  ...
  if (loading)
    return (
      <div className="dashboard-page-container status-message">
        Loading Dashboard...
      </div>
    );
  if (!alumniData)
    return (
      <div className="dashboard-page-container status-message error">
    ...
    <div className="dashboard-page-container">
      ...
      <div className="dashboard-tabs">
        ...
        <button
          ...
          className={`tab ${activeSection === "announcements" ? "active" : ""}`}
        >
      ...
      <main className="dashboard-main-content">
        {activeSection === "profile" && (
          <AlumniProfile alumniData={alumniData} />
        )}
        {activeSection === "announcements" && (
          <ManageAnnouncements alumniData={alumniData} />
        )}
